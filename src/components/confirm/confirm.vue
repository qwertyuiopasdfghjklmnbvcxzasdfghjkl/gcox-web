...
export default {
  ...
  computed: {
    ...
    dialogStyle () {
      return {
        width: `${this.width}vw`,
        height: this.height ? `${this.height}vh` : `auto`,
        'min-height': this.minHeight? `${this.minHeight}vh` : `auto`
      }
    }
  },
  created () {
...
.top{width:100%;height:30px;position:relative;background-color:#161f2f;justify-content:center;align-items:center;display:flex;color:#becbe8;}
.form{display:flex;justify-content:center;align-items:center;flex-flow:column;padding:30px;}
.content{color:#261003;font-size:16px;word-wrap:break-word;word-break:normal;width: 100%;}
.buttons{margin-top:20px;display:flex;align-items:center;}
.buttons-center{align-self:center;}
.buttons-right{align-self:flex-end;}
.button{display:flex;justify-content:center;align-items:center;min-width:80px;height:30px;line-height:30px;color:#fff;background-color:#0072fd;border-radius:4px;cursor:pointer; border:1px solid #0072fd;}
.button:hover{background-color:#0072fd;}
.button.cancel{margin-right:10px;color:#0072fd;background-color:transparent;}
.button.cancel:hover{color:#0072df;border-color:#0072fd;}
</style>
