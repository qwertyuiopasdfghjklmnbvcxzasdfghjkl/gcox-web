            ...
            <div class="inner">
                <ul class="item">
                    ...
                    <li>
                        ...
                        <section>{{item.cur_price.toString().toMoney()}} {{item.currency}}</section>
                    </li>
                    <li>
                        <section>
                        </section>
                        <section>
                            <button class="c-button c-button-normal c-button-blue" @click="deleteAd(item.ad_id)" v-if="item.status">
                                {{$t('otc_ad.otc_cancel_ad')}}<!--下架广告-->
                            </button>
                            <button class="c-button c-button-normal c-button-orange" @click="modifyAd(item.ad_id,item.symbol)" v-else>
  ...
  methods: {
    ...
    modifyAd (id,symbol) { // 修改广告
      otcApi.getPaySettings((res) => {
        this.$router.push({name:'otc-update', params:{orderId:id, symbol:symbol, myPayType: res.data.pay_type}})
      })
    },
  }
}
