...
export default {
	...
	computed:{
		...
		symbolList(){
		  return [...new Set(this.getMarketList.map(item=>{return item.baseSymbol}))].sort()
		},
		curProducts () {
		  ...
		  if(this.current==='favor'){
		  	if(this.getApiToken){
		  		datas = datas.filter(item=>{return item.collection})
		  	} else {
		  		datas = datas.filter(item=>{return this.favorSymbol.includes(item.market)})
		  	}
		  } else if(this.current==='all'){
		  	datas = datas.filter(item=>{return item.baseSymbol===this.baseSymbol})
		  }
		  return datas
		},
	},
